 ...
 * Notes: Dear God have mercy on my worthless soul.
 ****************************************************/
...
void catchSIGINT(int signo){	//Ctrl-C signal handler. SIGINT
}

void catchSIGUSR2(int signo){	//Custom signal handler. SIGUSR2
	exit(0);
}
...
void catchSIGTSTP(int signo){	//Ctrl-Z signal handler. SIGTSTP
	...
	if (fgmode == 0){		//le on.
		write(STDOUT_FILENO, messageOn_39, 39); fflush(stdout);
		fgmode = 1;
	}else {				//le off.
 ...
 ***************************/
	...
	while (pch !=0){		//PARSE THROUGH USER STRING AND ISOLATE COMMANDS
		...
		else if (commands[0] != NULL && strcmp(commands[0],"echo")==0){
...
//						execlp(commands[j], commands[j], "-a", NULL);
				...
				default:
					...
					if (strcmp(commands[0],"echo")==0){j++;}
					if (commands[j][0] == '&') { j++; }
					waitpid(Spawn_Children[spawnTracker], &process_exit_value,0);
